...
re-codded or modified by alinshan
to bring new ai
*/
...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to chandrappan voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('chandrappan') && conf.TALKING_CHANDRAPPAN !== 'true') {        
        ...
        var finm = message.message.replace('chandrappan', '').replace(' ', '')   
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_CHANDRAPPAN == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel chandrappan özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'chandrappan yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'chandrappan yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'chandrappan, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'chandrappan, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Chandrappan features. Turn your account into a ai chatbot!'
    already_on = 'Chandrappan artificial intelligence is already fully functional.'
    already_off = 'Chandrappan artificial intelligence is currently running semi-functional.'
    succ_on = 'Chandrappan Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'Chandrappan Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ chandrappan സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'chandrappan കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'chandrappan AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'chandrappan പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി chandrappan സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Asena.addCommand({ pattern: 'chandrappan ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.chandrappan on / off' }, (async (message, match) => {
    var chandrappan_status = `${conf.TALKING_CHANDRAPPAN}`
    if (match[1] == 'on') {
        if (chandrappan_status == 'true') {
            return await message.client.sendMessage(message.jid, '*' + already_on + '*', MessageType.text)
        }
        else {
            await heroku.patch(baseURI + '/config-vars', { 
                body: { 
                    ['TALKING_CHANDRAPPAN']: 'true'
                } 
            });
